	...
	err := wait.For(func(ctx context.Context) (bool, error) {
		...
		for _, deployment := range deploymentList.Items {
			...
			if deployment.Status.ReadyReplicas != *deployment.Spec.Replicas {
				notReadyDeployments = append(notReadyDeployments,
					fmt.Sprintf("%s (ready: %d/%d)",
						deploymentKey,
		...
		if len(notReadyDeployments) > 0 {
			t.Logf("Not ready deployments: %v", notReadyDeployments)
			return false, nil
		}
	...
	err := wait.For(func(ctx context.Context) (bool, error) {
		...
		for _, daemonSet := range daemonSetList.Items {
			...
			if daemonSet.Status.NumberReady != daemonSet.Status.DesiredNumberScheduled {
				notReadyDaemonSets = append(notReadyDaemonSets,
					fmt.Sprintf("%s (ready: %d/%d)",
						daemonSetKey,
		...
		if len(notReadyDaemonSets) > 0 {
			t.Logf("Not ready daemonsets: %v", notReadyDaemonSets)
			return false, nil
		}
...
func waitManagedChartsReady(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
	...
	err := wait.For(func(ctx context.Context) (bool, error) {
		...
		for _, chart := range managedCharts.Items {
			...
			if !ready {
				notReadyCharts = append(notReadyCharts, chartKey)
		...
		if len(notReadyCharts) > 0 {
			t.Logf("Not ready managed charts: %v", notReadyCharts)
			return false, nil
		}
	...
	err := wait.For(func(ctx context.Context) (bool, error) {
		...
		for _, kv := range kubevirtList.Items {
			...
			if kv.Status.Phase != kubevirtv1.KubeVirtPhaseDeployed {
				notReadyKubeVirts = append(notReadyKubeVirts, fmt.Sprintf("%s (phase: %s)", kvKey, kv.Status.Phase))
		...
		if len(notReadyKubeVirts) > 0 {
			t.Logf("Not ready KubeVirt resources: %v", notReadyKubeVirts)
			return false, nil
		}
